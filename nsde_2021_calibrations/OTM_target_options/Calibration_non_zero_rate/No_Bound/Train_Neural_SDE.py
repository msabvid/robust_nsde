...
class Net_LSV(nn.Module):
    ...
    def __init__(self, timegrid, strikes, device, n_maturities):
        ...
        """
        self.vanilla_hedge = Net_timegrid(dim=2, nOut=len(strikes), n_layers=2, vNetWidth=20, n_maturities=n_maturities, activation_output="softplus")
    ...
    def forward(self, S0, rate, z, MC_samples, ind_T, period_length,n_maturities): 
        ...
        S_old = ones * S0
        cv_vanilla = torch.zeros(S_old.shape[0], len(self.strikes),n_maturities, device=self.device)
        cv_exotic = torch.zeros(S_old.shape[0], 1 , device=self.device)
        pv_h = torch.zeros(len(self.strikes), device=self.device)
        ...
        cv_vanilla_fwd = torch.zeros(S_old.shape[0], len(self.strikes),n_maturities, device=self.device)
        ...
        for i in range(1, ind_T+1):
            ...
            for mat in range(n_maturities):
                 if mat>=idx_net:
                     cv_vanilla[:,:,mat] += cv_vanilla_fwd[:,:,mat] * dS.detach().repeat(1,len(self.strikes))  
...
def train_nsde(model,z_val,z_val_var,z_test,z_test_var,config):
    ...
    target_mat_T = torch.tensor(config["target_data"][:len(config["maturities"]),:len(config["strikes"])], device=device).float()
