        ...
        if epoch==0:
            ...
            try:
                for idxt, t in enumerate(maturities):
                    ...
                    for idxx, ( target, k) in enumerate(zip(van_target, K)):
                        if k>S0:
                            iv_target[idxx] = implied_volatility(target,  S=S0, K=k, r=rate, t=maturity_values[idxt]-1/365, flag="c")
                        else:
                            iv_target[idxx] = implied_volatility(target,  S=S0, K=k, r=rate, t=maturity_values[idxt]-1/365, flag="p")
            ...
            try:
                for idxt, t in enumerate(maturities):
                    ...
                    for idxx, (target, k) in enumerate(zip(van_target, K)):
                        if k>S0:
                            iv_target_temp = implied_volatility(target,  S=S0, K=k, r=rate, t=maturity_values[idxt]-1/365, flag="c")
                        else:
                            iv_target_temp = implied_volatility(target,  S=S0, K=k, r=rate, t=maturity_values[idxt]-1/365, flag="p")
                        iv = iv_target_temp+constant_shift
                        if k>S0:
                            inverse_vega_target[idxx] = constant_shift/(price_black_scholes(flag='c',S=S0,K=k,t=maturity_values[idxt]-1/365,r=rate,sigma=iv)-target)
                        else:
                            inverse_vega_target[idxx] = constant_shift/(price_black_scholes(flag='p',S=S0,K=k,t=maturity_values[idxt]-1/365,r=rate,sigma=iv)-target)
        ...
        with torch.no_grad():
                for idxt, t in enumerate(maturities):
                    ...
                    for idxx, (pred_temp, k) in enumerate(zip(van_pred_temp, K)):
                        try:
                            if k>S0:
                                iv[idxx] = implied_volatility(pred_temp,  S=S0, K=k, r=rate, t=maturity_values[idxt]-1/365, flag="c")
                            else:
                                iv[idxx] = implied_volatility(pred_temp,  S=S0, K=k, r=rate, t=maturity_values[idxt]-1/365, flag="p")
                        except:
                            pass
