...
GOOG_timegrids=zeros(n,8*max(max(maturities)));
GOOG_timegrids(1,2:(length(GOOG_cal_days)))=GOOG_cal_days(2:end)-1;
GOOG_timegrids(1,(length(GOOG_cal_days))+1)=GOOG_cal_days(end)
GOOG_granular = fill_timegrid(GOOG_timegrids(1,1:(length(GOOG_cal_days))+1));
len_temp = length(GOOG_granular(1,:));
GOOG_timegrids(1,1:len_temp)=GOOG_granular(1,:)
...
for i=2:n
    ...
    GOOG_timegrids(i,9:(length(GOOG_cal_days_temp)+7))=GOOG_cal_days_temp(2:end)-1;
    GOOG_timegrids(i,2)=1/8;
    GOOG_timegrids(i,3)=2/8;
    GOOG_timegrids(i,4)=3/8;
    GOOG_timegrids(i,5)=4/8;
    GOOG_timegrids(i,6)=5/8;
    GOOG_timegrids(i,7)=6/8;
    GOOG_timegrids(i,8)=7/8;
end
...
function [timegrid_granular] = fill_timegrid(timegrid_daily)

n = length(timegrid_daily(1,:));
timegrid_granular = zeros(1,8*n);
for i=1:n
    timegrid_granular(1,(i-1)*8+1)=timegrid_daily(i);
    for k=1:7
        timegrid_granular(1,8*(i-1)+1+k)=timegrid_daily(i)+k*1/8;
    end
...
[Put_ITM_idx,~,~] = intersect(Put_ITM_idx_1,Put_ITM_idx_2);
   ...
   plot_title = strcat('Maturity: T=', string(maturities(i)-1) , ' (d), ',' Date:  ', string(GOOG_day(Iday(1))),'.', string(GOOG_month(Iday(1))),'.' ,string(GOOG_year(Iday(1))));
   save_string = strcat('Date',string(GOOG_day(Iday(1))),string(GOOG_month(Iday(1))),string(GOOG_year(Iday(1))),'Maturity',string(maturities(i)-1));
   title(plot_title);
