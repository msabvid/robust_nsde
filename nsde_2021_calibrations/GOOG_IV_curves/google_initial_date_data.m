...
for i = 1:n
    ...
    [IV(i,:,:),IV_blend(i,:,:),VOL(i,:,:),K(i,:,:),Delta(i,:,:),ASK(i,:,:),BID(i,:,:)] = get_price_data(GOOG_cal_idx(i),GOOG_date,maturities(i,:),GOOG_S0(i),GOOG_mat,GOOG_IV,GOOG_VOL,GOOG_K,GOOG_BID,GOOG_ASK,GOOG_delta,number_strikes_for_each_maturity,GOOG_day,GOOG_month,GOOG_year,number_maturities_for_each_cal_day);
end
...
function [IV,IV_blend,VOL,K,Delta,ASK,BID] = get_price_data(day,GOOG_date,maturities,GOOG_S0,GOOG_mat,GOOG_IV,GOOG_VOL,GOOG_K,GOOG_BID,GOOG_ASK,GOOG_delta,number_strikes_for_each_maturity,GOOG_day,GOOG_month,GOOG_year,number_maturities_for_each_cal_day)
...
[Put_ITM_idx,~,~] = intersect(Put_ITM_idx_1,Put_ITM_idx_2);
  ...
  % [Imat,~]=find(GOOG_date(Iday)==maturities(i))
   ...
   [~,iv_spaps] = spaps(strikes_sorted, iv_blend_sorted,0.00025);
   IV(i,:)=iv_sorted;
   plot(strikes_sorted,iv_sorted, strikes_sorted,iv_blend_sorted)
   hold on
   plot(strikes_sorted_unique,iv_spaps,'--rs','LineWidth',2,...
                       'MarkerEdgeColor','k',...
                       'MarkerFaceColor','g',...
                       'MarkerSize',10)
   xlabel('Strike')
   ylabel('Implied Vol.')
   legend('Market IV Raw','Market IV blend', 'Market IV blend and smooth')
