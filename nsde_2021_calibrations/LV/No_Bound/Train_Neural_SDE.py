...
class Net_LV(nn.Module):
    ...
    def __init__(self, timegrid, strikes_call, device, n_maturities):
        ...
        self.net_Diffusion =  Net_timegrid(dim=2, nOut=1, n_layers=3, vNetWidth=150, n_maturities=n_maturities, activation_output="softplus")
        ...
        # exotic hedging strategy parameters
        self.exotic_hedge = Net_timegrid(dim=2, nOut=1, n_layers=3, vNetWidth=200, n_maturities=n_maturities)


    def forward(self, S0, rate, z_1, MC_samples, ind_T, period_length,cal_exotic_var,n_maturities): 
        # initialisation
        ones = torch.ones(MC_samples, 1, device=self.device)
        zeros = torch.zeros(MC_samples,1, device=self.device)
        S1_old = ones * S0
        cv_van_1 = torch.zeros(S1_old.shape[0], len(self.strikes_call),n_maturities, device=self.device)
        ...
        cv_fwd = torch.zeros(S1_old.shape[0], len(self.strikes_call),n_maturities, device=self.device)
        ...
        for i in range(1, ind_T+1):
            ...
            if cal_exotic_var==0:
                diffusion = self.net_Diffusion.forward_idx(idx_net, torch.cat([t,S1exp_old],1))
                for mat in range(n_maturities):
                    if mat>=idx_net:
                       cv_fwd[:,:,mat] = self.net_CV.forward_idx(mat, torch.cat([t,S1exp_old.detach()],1))
            if cal_exotic_var==1:
               with torch.no_grad():
                diffusion = self.net_Diffusion.forward_idx(idx_net, torch.cat([t,S1exp_old],1))
                for mat in range(n_maturities):
                    if mat>=idx_net:
                       cv_fwd[:,:,mat] = self.net_CV.forward_idx(mat, torch.cat([t,S1exp_old.detach()],1))
               # exotic_hedge = self.exotic_hedge.forward_idx(idx_net, torch.cat([t,S1exp_old.detach()],1)) # if exotic hedge depends on inst. vol.
            ...
            drift_1 =  (rate-0.5*(diffusion**2).reshape(MC_samples,1))
            drift_1c = (1+torch.abs(drift_1.detach())*torch.sqrt(h))

            # Those are used in grad calculation 
            diff_1 = diffusion.reshape(MC_samples,1) * dW_1
            # Normalisation constants in diffusion Tamed Euler scheme
            diff_1_c = (1+torch.abs(diffusion_const.reshape(MC_samples,1))*torch.sqrt(h))
            
            # Tamed Euler step
            S1exp_new = S1exp_old + drift_1*h/drift_1c + diff_1/diff_1_c 
            S1_new=torch.min(torch.exp(S1exp_new),S0*1000*ones)
            ...
            for mat in range(n_maturities):
                 if mat>=idx_net:
                     cv_van_1[:,:,mat] += cv_fwd[:,:,mat] * repeat_1 #cv_fwd[:,1:1+len(self.strikes_call)] * repeat_1
            if cal_exotic_var==1:
        ...
        # Apply control variate and calculate variance of estimates (used to train CV neural network)         
               for idx, strike in enumerate(self.strikes_call):
                  pv_1_cv = dis_fact*torch.clamp(S1_old-strike,0).squeeze(1).detach()-cv_van_1[:,idx,idx_net]
                  pv_1_cv_detach = dis_fact*torch.clamp(S1_old-strike,0).squeeze(1)-cv_van_1[:,idx,idx_net].detach()
...
def train_models(args):
    ...
    for idx, T in enumerate(maturities):
       ...
       params_SDE = list(model.net_Diffusion.parameters()) 
       ...
       scheduler_SDE_4 = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_SDE_4, 'min')
       ...
       for epoch in range(n_epochs):
            ...
            if epoch==0:
               ...
               # van_target_1 = target_mat_T[6:21].cpu().numpy()
                ...
                with torch.no_grad():
                    pv_1cv, _,_,exotic_prc_no_cv,exotic_prc_cv,_, _,martingale_test, put_atm, call_atm, put_call_parity_error = model(S0, rate, z_1, MC_samples_gen, T, period_length=16,cal_exotic_var=1,n_maturities=n_maturities)
                    _, var_pv_1cv_val,_,_,_,exotic_prc_var_no_cv_val, exotic_prc_var_cv_val,_,_,_,_ = model(S0, rate, z_1_var,MC_samples_var, T, period_length=16,cal_exotic_var=1,n_maturities=n_maturities)
            ...
            for i in range(0,20*batch_size, batch_size):
                ...
                if epoch%10==1 and epoch<250: 
                    model.net_CV.unfreeze()
                    model.net_Diffusion.freeze()
                    init_time = time.time()
                    _, var_pv_1cv,_, _,_,_, _, _, _, _, _ = model(S0, rate, batch_x1, batch_size,T, period_length=16,cal_exotic_var=0,n_maturities=n_maturities)
                ...
                if epoch%10!=1 and epoch<250:
                    model.net_Diffusion.unfreeze()
                    model.net_CV.freeze()
                    init_time = time.time()
                    pv_1cv, _,_, exotic_prc_no_cv,_,_, _, _, put_atm, call_atm, put_call_parity_error = model(S0, rate, batch_x1, batch_size,T, period_length=16,cal_exotic_var=0,n_maturities=n_maturities)
                    time_forward = time.time() - init_time
                    pred = torch.reshape(pv_1cv,(6,21))
                    MSE = loss_fn(torch.mul(pred,vega),torch.mul(target_mat_T,vega))
                    loss= -0*exotic_prc_no_cv + LAMBDA * MSE + c_2/2 * MSE**2  # we use exotic price without control variate in optimising payoff bound 
                    init_time = time.time()
                    itercount_2 +=1
                    loss.backward()
                    #if itercount_2%20==0:
                ...
                if epoch>=250:
                    ...
                    init_time = time.time()
                    _, _,_, _,_, _,exotic_prc_var_cv,_, _, _, _ = model(S0, rate, batch_x1, batch_size_hedge,T, period_length=16,cal_exotic_var=1,n_maturities=n_maturities)
                    time_forward = time.time() - init_time
            ...
            print('pred', pred)
            if epoch>=250:
               scheduler_SDE_4.step(exotic_prc_var_cv_val)
            loss_val = torch.sqrt(loss_fn(pred,target_mat_T))
            loss_vega = torch.sqrt(loss_fn(torch.mul(pred,vega),torch.mul(target_mat_T,vega)))
            loss_val_rel  = torch.sqrt(loss_fn(torch.div(pred,target_mat_T), torch.ones_like(pred)))
            print('validation {}, loss={:.10f}'.format(itercount, loss_val.item()))
            ...
            if epoch >= 250 and exotic_hedge_error< best_hedge_error: # train exotic control variate for remaining epochs and return best model
                 ...
                 checkpoint = {"state_dict":model.state_dict(),
                         ...
                         "target_mat_T": target_mat_T}
                 torch.save(checkpoint, filename)
            if (epoch>= 250 and exotic_hedge_error_change < 0.001) or epoch==274:
                  ...
                  z_test = torch.tensor(z_1).to(device=device).float()
                  z_test_var = torch.tensor(z_1_var).to(device=device).float()
                  with torch.no_grad():
                    pv_1cv, _,_,exotic_prc_no_cv, exotic_prc_cv,_, _,martingale_test, put_atm, call_atm, put_call_parity_error = model(S0, rate, z_test, MC_samples_gen, T, period_length=16,cal_exotic_var=1,n_maturities=n_maturities)
                    _, var_pv_1cv_val,_,_,_, exotic_prc_var_no_cv_val,exotic_prc_var_cv_val, _,_,_,_ = model(S0, rate, z_test_var,  MC_samples_var, T, period_length=16,cal_exotic_var=1,n_maturities=n_maturities)
                  print("pred_1:",pv_1cv)
                  print("target:", target_mat_T )
                  pred=torch.reshape(pv_1cv,(6,21))
                  print('pred', pred)
