...
class Net_LSV(nn.Module):
    ...
    def __init__(self, strikes, device, n_maturities):
        ...
        self.S_vol =  Net_timegrid(dim=4, nOut=1, n_layers=3, vNetWidth=200, n_maturities=n_maturities, activation_output="softplus")
        ...
        """
        self.vanilla_hedge = Net_timegrid(dim=3, nOut=len(strikes[0,:]), n_layers=2, vNetWidth=20, n_maturities=n_maturities, activation_output="softplus")

        # initialise stochastic volatility drift and diffusion neural networks
        self.V_drift = Net_timegrid(dim=2, nOut=1, n_layers=2, vNetWidth=20, n_maturities=n_maturities)
        self.V_vol = Net_timegrid(dim=2, nOut=1, n_layers=2, vNetWidth=20, n_maturities=n_maturities, activation_output="softplus")
        ...
        """
        self.exotic_hedge = Net_timegrid(dim=3, nOut=1, n_layers=2, vNetWidth=20, n_maturities=n_maturities)


    def forward(self, S0, rate_daily, z, MC_samples, ind_T,n_maturities, maturities, timegrid,gradient_count): 
        """
        ...
        for i in range(1, ind_T+1):
            ...
            price_diffusion = self.S_vol.forward_idx(idx_net, torch.cat([t,Slog_old,V_old,idx_input],1))
            
            # Evaluate vanilla hedging strategies at particular timestep
            for mat in range(n_maturities):
                if mat>=idx_net:
                    cv_vanilla_fwd[:,:,mat] = self.vanilla_hedge.forward_idx(mat, torch.cat([t,Slog_old.detach(),idx_input],1))
           
            exotic_hedge = self.exotic_hedge.forward_idx(idx_net, torch.cat([t,Slog_old.detach(),idx_input],1))
            
            if i in irand:
                V_new = V_old + self.V_drift.forward_idx(idx_net,torch.cat([V_old,idx_input],1)).reshape(MC_samples,1)*h + self.V_vol.forward_idx(idx_net,torch.cat([V_old,idx_input],1)).reshape(MC_samples,1)*dB
            else:
                V_new = V_old + self.V_drift.forward_idx(idx_net,torch.cat([V_old,idx_input],1)).detach().reshape(MC_samples,1)*h + self.V_vol.forward_idx(idx_net,torch.cat([V_old,idx_input],1)).detach().reshape(MC_samples,1)*dB
                
            price_diffusion_const = price_diffusion.detach()

            drift =  (rate_daily[i-1]-0.5*(price_diffusion**2).reshape(MC_samples,1))
            ...
            dS = torch.exp(-rate_daily[i]*timegrid[i])*S_new - torch.exp(-rate_daily[i-1]*timegrid[i-1])*S_old
...
def train_nsde(model,z_val,z_val_var,z_test,z_test_var,config):
    ...
    batch_steps_daily = config["maturities_daily"][-1] # this can be changed to end forward pass before the final maturity 
    ...
    for epoch in range(n_epochs):
        ...
        if epoch==0:
            ...
            constant_shift=0.001 # vega of each option is calculated by shifting corresponding IV by 1% 
           ...
           # except:
            ...
            with torch.no_grad():
                pv_h, _,pe_u,pe_h,_, _,martingale_test, put_atm, call_atm, put_call_parity_error = model(S0, rate_daily, z_val, MC_samples_price, batch_steps_daily,n_maturities=n_maturities,maturities=maturities_daily,timegrid=timegrid_daily,gradient_count=gradient_count)
                _, var_pv_h,_,_,pe_var_u, pe_var_h,_,_,_,_ = model(S0, rate_daily, z_val_var,MC_samples_var, batch_steps_daily,n_maturities=n_maturities,maturities=maturities_daily,timegrid=timegrid_daily,gradient_count=gradient_count)                 
        ...
        for i in range(0,20*batch_size, batch_size):
            ...
            if epoch%10==1: 
                ...
                init_time = time.time()
                batch_x1 = torch.randn(batch_size_hedge, batch_steps_daily, device=device)
                _, var_pv_h, _,_,_, _, _, _, _, _ = model(S0, rate_daily, batch_x1, batch_size_hedge,T_daily,n_maturities=n_maturities,maturities=maturities_daily,timegrid=timegrid_daily,gradient_count=gradient_count)
            ...
            if epoch%10!=1 and epoch%10!=2: 
                ...
                pv_h, _, _,pe_h,_, _, _, put_atm, call_atm, put_call_parity_error = model(S0, rate_daily, batch_x1, batch_size,T_daily,n_maturities=n_maturities, maturities=maturities_daily,timegrid=timegrid_daily,gradient_count=gradient_count)
                time_forward = time.time() - init_time
                pred = torch.reshape(pv_h,(n_maturities,n_strikes))
            ...
            if epoch%10==2: 
                ...
                init_time = time.time()
                batch_x1 = torch.randn(batch_size_hedge, batch_steps_daily, device=device)
                _, _, _,_, _,pe_var_h,_, _, _, _ = model(S0, rate_daily, batch_x1, batch_size_hedge,T_daily,n_maturities=n_maturities, maturities=maturities_daily,timegrid=timegrid_daily,gradient_count=gradient_count)
                time_forward = time.time() - init_time
                loss= pe_var_h
                init_time = t
                itercount +=1
        ...
        with torch.no_grad():
            pv_h_val, _,pe_u_val, pe_h_val,_, _,martingale_test, put_atm, call_atm, put_call_parity_error = model(S0, rate_daily, z_val, MC_samples_price, T_daily, n_maturities=n_maturities, maturities=maturities_daily,timegrid=timegrid_daily,gradient_count=gradient_count)
            _, var_pv_h_val,_,_, pe_var_u_val,pe_var_h_val, _,_,_,_ = model(S0, rate_daily, z_val_var,  MC_samples_var, T_daily, n_maturities=n_maturities, maturities=maturities_daily,timegrid=timegrid_daily,gradient_count=gradient_count)
        ...
        if iv_mean_error < best_IV_mean_error:
            ...
            print('IV mean error',iv_mean_error)
            filename = "Neural_SDE_validation_{}_maturity_{}_LSV_{}.pth.tar".format(type_bound,T_daily,seed)
        ...
        if  epoch==(n_epochs-1):
            checkpoint_str= "Neural_SDE_validation_{}_maturity_{}_LSV_{}.pth.tar".format(type_bound,T_daily,seed)

            checkpoint=torch.load(checkpoint_str)
            model.load_state_dict(checkpoint['state_dict'])
            model = model.to(device) 
            
            filename = "Neural_SDE_test_{}_maturity_{}_LSV_{}.pth.tar".format(type_bound,T_daily,seed)

            with torch.no_grad():
                pv_h_test,_,pe_u_test, pe_h_test,_, _,martingale_test, put_atm, call_atm, put_call_parity_error = model(S0, rate_daily, z_test, MC_samples_price, T_daily, n_maturities=n_maturities,maturities=maturities_daily,timegrid=timegrid_daily,gradient_count=gradient_count)
                _, var_pv_h_test,_,_, pe_var_u_test,pe_var_h_test, _,_,_,_ = model(S0, rate_daily, z_test_var,  MC_samples_var, T_daily, n_maturities=n_maturities,maturities=maturities_daily,timegrid=timegrid_daily,gradient_count=gradient_count)
...
if __name__ == '__main__':
    ...
    rates = 0.04 # np.load("rates.npy")
    mu = np.load("mu.npy")
    mu = [mu]
    print('strikes', strikes)

    maturity_values = [4/365]
    n_maturities = 1
    maturities_daily = [4]
    print('maturities_daily',maturities_daily)
    S0 = 529.876 # initial asset price 
    n_steps_daily =   maturities_daily[len( maturities_daily)-1]*24
    gradient_count = int(1*n_steps_daily) # use 100% of timesteps to evaluate gradient for backpropagation
    print('n_steps_daily',n_steps_daily)
    timegrid_daily = torch.linspace(0,maturity_values[0],n_steps_daily+1).to(device)
    print('timegrid_daily',timegrid_daily)
    rate_1_daily = torch.tensor(rates)*torch.ones(4*24+1)
    rate_daily = rate_1_daily 
    seed=901
