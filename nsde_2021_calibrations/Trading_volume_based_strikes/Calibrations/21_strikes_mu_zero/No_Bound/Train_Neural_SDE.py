...
class Net_LSV(nn.Module):
    ...
    """
    def __init__(self, timegrid, strikes, device, n_maturities):
    ...
    def forward(self, S0, rate, z, MC_samples, ind_T, period_length,n_maturities): 
        ...
        for i in range(1, ind_T+1):
            ...
            dS = torch.exp(-rate[i]*self.timegrid[i])*S_new - torch.exp(-rate[i-1]*self.timegrid[i-1])*S_old
...
if __name__ == '__main__':
    ...
    timegrid_1 = torch.linspace(0,maturity_values[0],17).to(device)
    timegrid_2 = torch.linspace(maturity_values[0],maturity_values[1],17).to(device)
    timegrid_3 = torch.linspace(maturity_values[1],maturity_values[2],17).to(device)
    timegrid_4 = torch.linspace(maturity_values[2],maturity_values[3],17).to(device)
    timegrid_5 = torch.linspace(maturity_values[3],maturity_values[4],17).to(device)
    timegrid_6 = torch.linspace(maturity_values[4],maturity_values[5],17).to(device)
    timegrid = torch.cat([timegrid_1,timegrid_2[1:17],timegrid_3[1:17],timegrid_4[1:17],timegrid_5[1:17],timegrid_6[1:17]],0)
    rate_1 = rates[0]*torch.ones_like(timegrid_1[1:17])
    ...
    rate = 0*torch.cat([rate_1,rate_2,rate_3,rate_4,rate_5,rate_6],0) # piecewise constant interest rate for each maturity
    seed=901

    torch.manual_seed(seed) # fixed for reproducibility
    model = Net_LSV(timegrid=timegrid, strikes=strikes, device=device, n_maturities=n_maturities)
    model.to(device)
