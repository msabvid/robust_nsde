        ...
        if epoch==0:
            ...
            try:
                for idxt, t in enumerate(maturities):
                    ...
                    for idxx, ( target, k) in enumerate(zip(van_target, K)):
                        iv_target[idxx] = implied_volatility(target,  S=S0, K=k, r=rate, t=t/T, flag="c") 
            ...
            try:
                for idxt, t in enumerate(maturities):
                    van_target = target_mat_T[idxt,::].cpu().numpy() 
                    for idxx, (target, k) in enumerate(zip(van_target, K)):
                        iv_target_temp = implied_volatility(target, S=S0, K=k, r=rate, t=t/T, flag="c") 
                        iv = iv_target_temp+constant_shift
                        inverse_vega_target[idxx] = constant_shift/(price_black_scholes(flag='c',S=S0,K=k,t=t/T,r=rate,sigma=iv)-target)
        ...
        with torch.no_grad():
                for idxt, t in enumerate(maturities):
                    ...
                    for idxx, (pred_temp, k) in enumerate(zip(van_pred_temp, K)):
                        try:
                            iv[idxx] = implied_volatility(pred_temp,  S=S0, K=k, r=rate, t=t/T, flag="c")
                        except:
                            pass
