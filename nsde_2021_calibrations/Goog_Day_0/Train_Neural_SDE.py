...
class Net_LSV(nn.Module):
    ...
    def forward(self, S0, rate, z, MC_samples, ind_T,n_maturities, maturities,maturity_exotic, strikes, timegrid,gradient_count): 
        """
        ...
        for i in range(1, ind_T+1):
            ...
            idx_net = [k for k in range(n_maturities) if timegrid[i]*365 <= maturities[k]] # maturities not necessarily uniform
            idx_net = idx_net[0]
            ...
            if args.hedge_exotic and args.lookback_exotic:
                exotic_hedge = self.exotic_hedge_lookback.forward_idx(0, torch.cat([t,Slog_old.detach()],1))
                
            if args.hedge_exotic and args.straddle_exotic:
                exotic_hedge = self.exotic_hedge_straddle.forward_idx(0, torch.cat([t,Slog_old.detach()],1))    
            ...
            if int(timegrid[i]*365+0.0001) in maturities: 
                dis_fact = torch.exp(-rate[idx_net]*timegrid[i])
            ...
            if int(timegrid[i]*365+0.0001)==maturity_exotic:  
                ...
                if args.hedge_exotic:
                    if args.lookback_exotic:
                        ...
                        exotic_payoff = torch.clamp(S_old.detach()-S0,0)+torch.clamp(-S_old.detach()+S0,0)
...
def train_nsde(model,z_val,z_val_var,z_test,z_test_var,config):
    ...
    optimizer_SDE = torch.optim.Adam(params_SDE,lr=50*learning_rate)
    ...
    target_mat_T = torch.tensor(config["target_data"][:len(config["maturity_values"]),:len(config["strikes"][0,:])], device=device).float()
    ...
    for epoch in range(n_epochs):
        ...
        if epoch==0:
            ...
            try:
                for idxt, t in enumerate(maturities_daily):
                    ...
                    for idxx, ( target, k) in enumerate(zip(van_target, K[idxt,:])):
                        if k>S0: 
                            iv_target[idxx] = implied_volatility(target,  S=S0, K=k, r=rate[idxt].cpu().numpy(), t=maturity_values[idxt]-1/365, flag="c")
                        else:
                            iv_target[idxx] = implied_volatility(target,  S=S0, K=k, r=rate[idxt].cpu().numpy(), t=maturity_values[idxt]-1/365, flag="p")
            ...
            try:
                for idxt, t in enumerate(maturities_daily):
                    ...
                    for idxx, (target, k) in enumerate(zip(van_target, K[idxt,:])):
                        if k>S0:
                            iv_target_temp = implied_volatility(target,  S=S0, K=k, r=rate[idxt].cpu().numpy(), t=maturity_values[idxt]-1/365, flag="c")
                        else:
                            iv_target_temp = implied_volatility(target,  S=S0, K=k, r=rate[idxt].cpu().numpy(), t=maturity_values[idxt]-1/365, flag="p")
                        iv = iv_target_temp+constant_shift
                        if k>S0:
                            inverse_vega_target[idxx] = constant_shift/(price_black_scholes(flag='c',S=S0,K=k,t=maturity_values[idxt]-1/365,r=rate[idxt].cpu().numpy(),sigma=iv)-target)
                        else:
                            inverse_vega_target[idxx] = constant_shift/(price_black_scholes(flag='p',S=S0,K=k,t=maturity_values[idxt]-1/365,r=rate[idxt].cpu().numpy(),sigma=iv)-target)
            ...
            with torch.no_grad():
                if args.hedge_exotic: 
                    _, _,pe_u, pe_h,_, _,_, _, _, _ =   model(S0, rate, z_val, MC_samples_price, batch_steps_daily,n_maturities=n_maturities,maturities=maturities_daily,maturity_exotic=maturity_exotic_daily,strikes=strikes,timegrid=timegrid_daily,gradient_count=gradient_count)
                    _, _,_,_, pe_var_u,pe_var_h, _,_,_,_ = model(S0, rate, z_val_var,MC_samples_var, batch_steps_daily,n_maturities=n_maturities,maturities=maturities_daily,maturity_exotic=maturity_exotic_daily,strikes=strikes,timegrid=timegrid_daily,gradient_count=gradient_count)
                pv_h, _,_, _,_, _,martingale_test, put_atm, call_atm, put_call_parity_error =   model(S0, rate, z_val, MC_samples_price, batch_steps_daily,n_maturities=n_maturities,maturities=maturities_daily,maturity_exotic=maturity_exotic_daily,strikes=strikes,timegrid=timegrid_daily,gradient_count=gradient_count)
                _, var_pv_h,_,_, _, _, _,_,_,_ = model(S0, rate, z_val_var,MC_samples_var, batch_steps_daily,n_maturities=n_maturities,maturities=maturities_daily,maturity_exotic=maturity_exotic_daily,strikes=strikes,timegrid=timegrid_daily,gradient_count=gradient_count)                 
        ...
        for i in range(0,20*batch_size, batch_size):
            ...
            if epoch%10==1 and args.hedge_exotic==False: 
                ...
                _, var_pv_h, _,_,_, _, _, _, _, _ = model(S0, rate, batch_x1, batch_size_hedge,T,n_maturities=n_maturities,maturities=maturities,maturity_exotic=maturity_exotic,strikes=strikes,timegrid=timegrid,gradient_count=gradient_count)
            ...
            if epoch%10!=1 and args.hedge_exotic==False: 
                ...
                pv_h, _, _,_,_, _, _, put_atm, call_atm, put_call_parity_error = model(S0, rate, batch_x1, batch_size,T,n_maturities=n_maturities, maturities=maturities,maturity_exotic=maturity_exotic,strikes=strikes,timegrid=timegrid,gradient_count=gradient_count)
                time_forward = time.time() - init_time
                pred = torch.reshape(pv_h,(n_maturities,n_strikes))
            ...
            if args.hedge_exotic: 
                ...
                init_time = time.time()
                batch_x1 = torch.randn(batch_size_hedge, batch_steps, device=device)
                _, _, _,pe_h, _,pe_var_h,_, _, _, _ = model(S0, rate, batch_x1, batch_size_hedge,T,n_maturities=n_maturities, maturities=maturities,maturity_exotic=maturity_exotic,strikes=strikes,timegrid=timegrid,gradient_count=gradient_count)
                time_forward = time.time() - init_time
                loss= pe_var_h
                init_time = t
                itercount +=1
        ...
        with torch.no_grad():
            if args.hedge_exotic: 
                _, _,pe_u_val, pe_h_val,_, _,_, _, _, _ = model(S0, rate, z_val, MC_samples_price, T_daily, n_maturities=n_maturities, maturities=maturities_daily,maturity_exotic=maturity_exotic_daily,strikes=strikes,timegrid=timegrid_daily,gradient_count=gradient_count)
                _, _,_,_, pe_var_u_val,pe_var_h_val, _,_,_,_ = model(S0, rate, z_val_var,  MC_samples_var, T_daily, n_maturities=n_maturities, maturities=maturities_daily,maturity_exotic=maturity_exotic_daily,strikes=strikes,timegrid=timegrid_daily,gradient_count=gradient_count)
            pv_h_val, _,_, _,_, _,martingale_test, put_atm, call_atm, put_call_parity_error = model(S0, rate, z_val, MC_samples_price, T_daily, n_maturities=n_maturities,maturity_exotic=maturity_exotic_daily, maturities=maturities_daily,strikes=strikes,timegrid=timegrid_daily,gradient_count=gradient_count)
            _, var_pv_h_val,_,_, _,_, _,_,_,_ = model(S0, rate, z_val_var,  MC_samples_var, T_daily, n_maturities=n_maturities, maturities=maturities_daily,maturity_exotic=maturity_exotic_daily,strikes=strikes,timegrid=timegrid_daily,gradient_count=gradient_count)
        ...
        with torch.no_grad():
                for idxt, t in enumerate(maturities_daily):
                    ...
                    for idxx, (pred_temp, k) in enumerate(zip(van_pred_temp, K[idxt,:])):
                        try:
                            if k>S0:
                                iv[idxx] = implied_volatility(pred_temp,  S=S0, K=k, r=rate[idxt].cpu().numpy(), t=maturity_values[idxt]-1/365, flag="c")
                            else:
                                iv[idxx] = implied_volatility(pred_temp,  S=S0, K=k, r=rate[idxt].cpu().numpy(), t=maturity_values[idxt]-1/365, flag="p")
                        except:
                            pass
        ...
        if iv_mean_error < best_IV_mean_error and args.hedge_exotic==False:
            ...
            filename = "NSDE_val_LSV_mat_{}_seed_{}_calib_day_{}.pth.tar".format(T_daily,seed,cal_day)
        ...
        if  args.hedge_exotic and pe_var_h_val < best_hedge_error:
            ...
            print('IV best error',iv_mean_error)
            if args.lookback_exotic:
                filename = "NSDE_val_hedge_lookback_maturity_{}_seed_{}_calib_day_{}.pth.tar".format(T_daily,seed,cal_day)
                checkpoint = {"state_dict":model.state_dict(),
                         "T":T_daily,
                         "pred":pred,
                         "MSE": loss_val,
            ...
            if args.straddle_exotic:
                filename = "NSDE_val_hedge_straddle_maturity_{}_seed_{}_calib_day_{}.pth.tar".format(T_daily,seed,cal_day)
                checkpoint = {"state_dict":model.state_dict(),
                         "T":T_daily,
                         "pred":pred,
                         "MSE": loss_val,
        ...
        if  epoch==(n_epochs-1):
            ...
            if args.hedge_exotic==False:
                checkpoint_str= "NSDE_val_LSV_mat_{}_seed_{}_calib_day_{}.pth.tar".format(T_daily,seed,cal_day)
            if args.lookback_exotic==True and args.hedge_exotic==True:
                checkpoint_str= "NSDE_val_hedge_lookback_maturity_{}_seed_{}_calib_day_{}.pth.tar".format(T_daily,seed,cal_day)
            if args.straddle_exotic==True and args.hedge_exotic==True:
                checkpoint_str= "NSDE_val_hedge_straddle_maturity_{}_seed_{}_calib_day_{}.pth.tar".format(T_daily,seed,cal_day)   
            ...
            if args.hedge_exotic==False:
                filename = "NSDE_test_LSV_mat_{}_seed_{}_calib_day_{}.pth.tar".format(T_daily,seed,cal_day)
            if args.lookback_exotic==True and args.hedge_exotic==True:
                checkpoint_str= "NSDE_test_hedge_lookback_maturity_{}_seed_{}_calib_day_{}.pth.tar".format(T_daily,seed,cal_day)
            if args.straddle_exotic==True and args.hedge_exotic==True:
                checkpoint_str= "NSDE_test_hedge_straddle_maturity_{}_seed_{}_calib_day_{}.pth.tar".format(T_daily,seed,cal_day)   

            with torch.no_grad():
                if args.hedge_exotic: 
                    _, _,pe_u_test, pe_h_test,_, _,_, _, _, _ =   model(S0, rate, z_test, MC_samples_price, T_daily, n_maturities=n_maturities,maturities=maturities_daily,maturity_exotic=maturity_exotic_daily,strikes=strikes,timegrid=timegrid_daily,gradient_count=gradient_count)
                    _, _,_,_, pe_var_u_test,pe_var_h_test, _,_,_,_ = model(S0, rate, z_test_var,  MC_samples_var, T_daily, n_maturities=n_maturities,maturities=maturities_daily,maturity_exotic=maturity_exotic_daily,strikes=strikes,timegrid=timegrid_daily,gradient_count=gradient_count)
                pv_h_test, _,_, _,_, _,martingale_test, put_atm, call_atm, put_call_parity_error =  model(S0, rate, z_test, MC_samples_price, T_daily, n_maturities=n_maturities,maturities=maturities_daily,maturity_exotic=maturity_exotic_daily,strikes=strikes,timegrid=timegrid_daily,gradient_count=gradient_count)
                _, var_pv_h_test,_,_, _, _, _,_,_,_ = model(S0, rate, z_test_var,  MC_samples_var, T_daily, n_maturities=n_maturities,maturities=maturities_daily,maturity_exotic=maturity_exotic_daily,strikes=strikes,timegrid=timegrid_daily,gradient_count=gradient_count)                 
...
if __name__ == '__main__':
    ...
    S0 = torch.tensor(np.load("GOOG_S0.npy").astype('float32')).to(device) # initial asset price 
    print('initial price', S0)

    timegrid_daily = np.load("timegrid.npy")
    n_steps_daily =  len(timegrid_daily)
    timegrid_daily = torch.tensor(timegrid_daily).to(device)#.cpu()
    rate = (torch.ones_like(timegrid_daily)*torch.tensor(rates).to(device)).type(torch.float32)
    seed=456
   ...
   # for name, param in model.named_parameters():
    ...
    z_val = np.random.normal(size=(MC_samples_price, n_steps_daily))
    z_val_var = np.random.normal(size=(MC_samples_var, n_steps_daily))
    np.random.seed(1234567) # fixed seed for test set model is evaluated after calibration completes 
    z_test = np.random.normal(size=(MC_samples_price, n_steps_daily))
    z_test_var = np.random.normal(size=(MC_samples_var, n_steps_daily))
    ...
    model_SDE_lookback_hedge = train_nsde(model_SDE, z_val,z_val_var,z_test,z_test_var, CONFIG_Exotic_Hedge)
    ...
    model_SDE_straddle_hedge = train_nsde(model_SDE, z_val,z_val_var,z_test,z_test_var, CONFIG_Exotic_Hedge)
