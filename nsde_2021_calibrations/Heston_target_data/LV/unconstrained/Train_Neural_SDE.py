...
class Net_LV(nn.Module):
    ...
    def __init__(self, timegrid, strikes_call, device, n_maturities):
        ...
        self.master =  Net_timegrid(dim=2, nOut=1+1*len(strikes_call), n_layers=3, vNetWidth=120, n_maturities=n_maturities, activation_output="softplus")

        # Exotic hedging strategy neural network
    ...
    def forward(self, S0, rate, z_1, MC_samples, ind_T, period_length,cal_exotic_var): 
        ...
        for i in range(1, ind_T+1):
            ...
            if cal_exotic_var==0:
                diffusion = self.master.forward_idx(idx_net, torch.cat([t,S1exp_old],1))
            if cal_exotic_var==1:
               with torch.no_grad():
                  diffusion = self.master.forward_idx(idx_net, torch.cat([t,S1exp_old],1))
            ...
            S1_new=torch.min(torch.exp(S1exp_new),1000*ones)
...
def train_models(args):
    ...
    for idx, T in enumerate(maturities):
       ...
       if idx==0:
          T=96 # simulate paths for the 12 month horizon
       else:
       ...
       params_SDE = list(model.master.parameters()) 
       ...
       for epoch in range(n_epochs):
                  ...
                  try:
                   for idxt, t in enumerate(maturities):
                        ...
                        for idxx, ( target, k) in enumerate(zip(van_target_1, K)):
                            iv_target_1[idxx] = implied_volatility(target,  S=S0, K=k, r=0.025, t=t/96, flag="c") 
                ...
                with torch.no_grad():
                    ...
                    try:
                        for idxt, t in enumerate(maturities):
                            van_target_1 = target_mat_T[idxt,::].cpu().numpy() 
                            for idxx, (target, k) in enumerate(zip(van_target_1, K)):
                                iv_target_11 = implied_volatility(target, S=S0, K=k, r=0.025, t=t/96, flag="c") 
                                iv_1 = iv_target_11+constant_shift
                                vega_target_1[idxx] = constant_shift/(prc(flag='c',S=S0,K=k,t=t/96,r=0.025,sigma=iv_1)-target)
            ...
            for i in range(0,20*batch_size, batch_size):
                ...
                if epoch%2==1 and epoch<250:
                    MSE = loss_fn(torch.mul(pred,vega),torch.mul(target_mat_T,vega))
                    loss= 0*exotic_prc_no_cv + LAMBDA_2 * MSE + c_2/2 * MSE**2  # we use exotic price without control variate in optimising payoff bound 
                    init_time = time.time()
                    itercount_2 +=1
                    loss.backward()
                    if itercount_2%20==0:
            ...
            with torch.no_grad():
                   for idxt, t in enumerate(maturities):
                        ...
                        for idxx, (pred_1, k) in enumerate(zip(van_pred_11, K)):
                          try:
                            iv_1[idxx] = implied_volatility(pred_1,  S=S0, K=k, r=0.025, t=t/96, flag="c")
                          except:
                            pass
