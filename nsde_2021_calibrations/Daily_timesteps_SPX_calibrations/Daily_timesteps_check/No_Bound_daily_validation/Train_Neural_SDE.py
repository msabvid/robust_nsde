...
class Net_LSV(nn.Module):
    ...
    """
    def __init__(self, strikes, device, n_maturities):
        ...
        """
        self.vanilla_hedge = Net_timegrid(dim=2, nOut=len(strikes[0,:]), n_layers=2, vNetWidth=20, n_maturities=n_maturities, activation_output="softplus")
    ...
    def forward(self, S0, rate, z, MC_samples, ind_T,n_maturities, maturities, timegrid): 
        """
        ...
        S_old = ones * S0
        cv_vanilla = torch.zeros(S_old.shape[0], len(self.strikes[0,:]),n_maturities, device=self.device)
        cv_exotic = torch.zeros(S_old.shape[0], 1 , device=self.device)
        pv_h = torch.zeros(len(self.strikes[0,:]), device=self.device)
        ...
        cv_vanilla_fwd = torch.zeros(S_old.shape[0], len(self.strikes[0,:]),n_maturities, device=self.device)
        ...
        for i in range(1, ind_T+1):
            idx_net = [k for k in range(n_maturities) if i <= maturities[k]] # maturities not necessarily uniform
            idx_net = idx_net[0]
            ...
            drift =  (rate[i-1]-0.5*(price_diffusion**2).reshape(MC_samples,1))
            ...
            dS = torch.exp(-rate[i]*timegrid[i])*S_new - torch.exp(-rate[i-1]*timegrid[i-1])*S_old
           
            # Evaluate stoch. integrals (w.r.t. discounted price process) corresponding to control variates
            for mat in range(n_maturities):
                 if mat>=idx_net:
                     cv_vanilla[:,:,mat] += cv_vanilla_fwd[:,:,mat] * dS.detach().repeat(1,len(self.strikes[0,:]))  
            ...
            if int(i) in maturities: 
                dis_fact = torch.exp(-mu[idx_net]*timegrid[i])
                for idx, strike in enumerate(self.strikes[idx_net,:]):
                  if strike>S0:  
            ...
            if int(i)==maturities[n_maturities-1]:  
...
def train_nsde(model,z_val,z_val_var,z_test,z_test_var,z_test_daily,z_test_var_daily,config):
    ...
    batch_steps = config["maturities"][-1] # this can be changed to end forward pass before the final maturity 
    batch_steps_daily = config["maturities_daily"][-1]
    T=config["maturities"][-1]
    T_daily=config["maturities_daily"][-1]
    learning_rate =config["learning_rate"]
    K = config["strikes"]
    rate = config["interest_rate"]
    rate_daily = config["interest_rate_daily"]
    ...
    target_mat_T = torch.tensor(config["target_data"][:len(config["maturities"]),:len(config["strikes"][0,:])], device=device).float()
    ...
    for epoch in range(n_epochs):
        ...
        if epoch==0:
            ...
            try:
                for idxt, t in enumerate(maturities):
                    ...
                    for idxx, ( target, k) in enumerate(zip(van_target, K[idxt,:])):
                        if k>S0:
                            iv_target[idxx] = implied_volatility(target,  S=S0, K=k, r=mu[idxt], t=maturity_values[idxt]-1/365, flag="c")
                        else:
                            iv_target[idxx] = implied_volatility(target,  S=S0, K=k, r=mu[idxt], t=maturity_values[idxt]-1/365, flag="p")
            ...
            try:
                for idxt, t in enumerate(maturities):
                    ...
                    for idxx, (target, k) in enumerate(zip(van_target, K[idxt,:])):
                        if k>S0:
                            iv_target_temp = implied_volatility(target,  S=S0, K=k, r=mu[idxt], t=maturity_values[idxt]-1/365, flag="c")
                        else:
                            iv_target_temp = implied_volatility(target,  S=S0, K=k, r=mu[idxt], t=maturity_values[idxt]-1/365, flag="p")
                        iv = iv_target_temp+constant_shift
                        if k>S0:
                            inverse_vega_target[idxx] = constant_shift/(price_black_scholes(flag='c',S=S0,K=k,t=maturity_values[idxt]-1/365,r=mu[idxt],sigma=iv)-target)
                        else:
                            inverse_vega_target[idxx] = constant_shift/(price_black_scholes(flag='p',S=S0,K=k,t=maturity_values[idxt]-1/365,r=mu[idxt],sigma=iv)-target)
            ...
            with torch.no_grad():
                pv_h, _,pe_u,pe_h,_, _,martingale_test, put_atm, call_atm, put_call_parity_error = model(S0, rate, z_val, MC_samples_price, batch_steps,n_maturities=n_maturities,maturities=maturities,timegrid=timegrid)
                _, var_pv_h,_,_,pe_var_u, pe_var_h,_,_,_,_ = model(S0, rate, z_val_var,MC_samples_var, batch_steps,n_maturities=n_maturities,maturities=maturities,timegrid=timegrid)                 
            pred=torch.reshape(pv_h,(n_maturities,n_strikes))
            print("Model Option Prices at Initialisation:",pred)
            print("Target Option OTM Prices:", target_mat_T )
        ...
        for i in range(0,20*batch_size, batch_size):
            ...
            if epoch%10==1: 
                ...
                _, var_pv_h, _,_,_, _, _, _, _, _ = model(S0, rate, batch_x1, batch_size_hedge,T,n_maturities=n_maturities,maturities=maturities,timegrid=timegrid)
            ...
            if epoch%10!=1 and epoch%10!=2: 
                ...
                pv_h, _, _,pe_h,_, _, _, put_atm, call_atm, put_call_parity_error = model(S0, rate, batch_x1, batch_size,T,n_maturities=n_maturities, maturities=maturities,timegrid=timegrid)
                time_forward = time.time() - init_time
                pred = torch.reshape(pv_h,(n_maturities,n_strikes))
            ...
            if epoch%10==2: 
                ...
                init_time = time.time()
                batch_x1 = torch.randn(batch_size_hedge, batch_steps, device=device)
                _, _, _,_, _,pe_var_h,_, _, _, _ = model(S0, rate, batch_x1, batch_size_hedge,T,n_maturities=n_maturities, maturities=maturities,timegrid=timegrid)
                time_forward = time.time() - init_time
                loss= pe_var_h
                init_time = t
                itercount +=1
        ...
        with torch.no_grad():
            pv_h_val, _,pe_u_val, pe_h_val,_, _,martingale_test, put_atm, call_atm, put_call_parity_error = model(S0, rate, z_val, MC_samples_price, T, n_maturities=n_maturities, maturities=maturities,timegrid=timegrid)
            _, var_pv_h_val,_,_, pe_var_u_val,pe_var_h_val, _,_,_,_ = model(S0, rate, z_val_var,  MC_samples_var, T, n_maturities=n_maturities, maturities=maturities,timegrid=timegrid)
        ...
        with torch.no_grad():
                for idxt, t in enumerate(maturities):
                    ...
                    for idxx, (pred_temp, k) in enumerate(zip(van_pred_temp, K[idxt,:])):
                        try:
                            if k>S0:
                                iv[idxx] = implied_volatility(pred_temp,  S=S0, K=k, r=mu[idxt], t=maturity_values[idxt]-1/365, flag="c")
                            else:
                                iv[idxx] = implied_volatility(pred_temp,  S=S0, K=k, r=mu[idxt], t=maturity_values[idxt]-1/365, flag="p")
                        except:
                            pass
        ...
        with torch.no_grad():
            pv_h_val, _,pe_u_val, pe_h_val,_, _,martingale_test, put_atm, call_atm, put_call_parity_error = model(S0, rate_daily, z_test_daily, MC_samples_price, T_daily, n_maturities=n_maturities, maturities=maturities_daily,timegrid=timegrid_daily)
            _, var_pv_h_val,_,_, pe_var_u_val,pe_var_h_val, _,_,_,_ = model(S0, rate_daily, z_test_var_daily, MC_samples_var, T_daily, n_maturities=n_maturities, maturities=maturities_daily,timegrid=timegrid_daily)
        pred=torch.reshape(pv_h_val,(n_maturities,n_strikes))
        print("Model Option Prices:",pred)
        print("Target Option OTM Prices:", target_mat_T )
        loss_val = torch.sqrt(loss_fn(pred,target_mat_T))
        ...
        with open("Price_Call_and_Price_Put_6_Month_ATM_Daily.txt","a") as f:
            f.write("{:.4f},{:.4f}\n".format(call_atm.item(),put_atm.item() ))
            
        with open("Price_Exotic_Hedged_and_Price_Exotic_Unhedged_Daily.txt","a") as f:
            f.write("{:.4f},{:.4f}\n".format(pe_h_val.item(),pe_u_val.item() ))
            
        with open("Sum_Variances__Vanilla_Options_Variance_Exotic_Hedged_Variance_Exotic_Unhedged_Daily.txt","a") as f:
            f.write("{:.4f},{:.4f},{:.4f}\n".format(torch.sum(var_pv_h_val).item(),pe_var_h_val.item(),pe_var_u_val.item() ))
            
        with open("Loss_MSE_Loss_REL_Loss_Vega_Weighted_Daily.txt","a") as f:
            f.write("{:.4f},{:.4f},{:.4f}\n".format(loss_val.item(),loss_val_rel.item(),loss_vega.item() ))
            
        with open("Put_Call_Parity_Error_and_Martingale_Test_Daily.txt","a") as f:
            f.write("{:.4f},{:.4f}\n".format(put_call_parity_error.item(),martingale_test.item() ))
            
        print('Test Daily: Mean Square Error {}, loss={:.10f}'.format(itercount, loss_val.item()))
        print('Test Daily: Relative Error {}, loss={:.10f}'.format(itercount, loss_val_rel.item()))
        print('Test Daily: Iverse Vega Weighted MSE {}, loss={:.10f}'.format(itercount, loss_vega.item()))   
        print('Test Daily: Sum of Variances of Vanilla OTM Options:', torch.sum(var_pv_h_val))
        print('Test Daily: Variance of Hedged Exotic Option:', pe_var_h_val)
        print('Test Daily: Variance of Unhedged Exotic Option:', pe_var_u_val)
        print('Test Daily: Price of Hedged Exotic Option:', pe_h_val)
        print('Test Daily: Price of Unhedged Exotic Option:', pe_u_val)
        print('Test Daily: Martingale Test:', martingale_test)
        print('Test Daily: ATM 6-Month Call Price:', call_atm)
        print('Test Daily: ATM 6-Month Put Price:', put_atm)
        print('Test Daily: ATM 6-Month Put-Call Parity Error:', put_call_parity_error)
        ...
        iv_mean_error= loss_fn(iv_out,iv_target_out).cpu().numpy() 
        iv_infinity_error = np.max(np.absolute(iv_out.cpu().numpy()-iv_target_out.cpu().numpy()))
        with open("IV_Mean_Infinity_Errors_Daily.txt","a") as f:
             f.write("{:.4f},{:.4f}\n".format(iv_mean_error.item(),iv_infinity_error.item())) 
            
        print('Test Daily: Implied Volatility Model', iv_out)
        print('Test Daily: Implied Volatility Target', iv_target_out)
        print('Test Daily: Implied Volatility Mean Square Error', iv_mean_error)
        print('Test Daily: Implied Volatility Infinity_error', iv_infinity_error)
   
        filename = "Neural_SDE_test_daily_{}_maturity_{}_LSV_{}.pth.tar".format(type_bound,T,seed)
        checkpoint = {"T":T,
                         "pred":pred,
                         "MSE": loss_val,
                         "exotic_hedge_error": pe_var_h_val,
        ...
        if  epoch==(n_epochs-1):
            ...
            with torch.no_grad():
                pv_h_test,_,pe_u_test, pe_h_test,_, _,martingale_test, put_atm, call_atm, put_call_parity_error = model(S0, rate, z_test, MC_samples_price, T, n_maturities=n_maturities,maturities=maturities,timegrid=timegrid)
                _, var_pv_h_test,_,_, pe_var_u_test,pe_var_h_test, _,_,_,_ = model(S0, rate, z_test_var,  MC_samples_var, T, n_maturities=n_maturities,maturities=maturities,timegrid=timegrid)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_bound', action='store_true', default=True)
    ...
    # Set up training   
    data = np.load("target_OTM_prices.npy")
    print('data',data) 
    ...
    maturities_daily = [int(item) for item in 365*maturity_values]
    S0 = 3.22122 # initial asset price 
    n_steps = period_length*n_maturities
    maturities =  range(period_length, n_steps+1, period_length)
    n_steps_daily =   maturities_daily[len( maturities_daily)-1]
    
    
    timegrid_1 = torch.linspace(0,maturity_values[0],period_length+1).to(device)
    timegrid_2 = torch.linspace(maturity_values[0],maturity_values[1],period_length+1).to(device)
    timegrid_3 = torch.linspace(maturity_values[1],maturity_values[2],period_length+1).to(device)
    timegrid_4 = torch.linspace(maturity_values[2],maturity_values[3],period_length+1).to(device)
    timegrid_5 = torch.linspace(maturity_values[3],maturity_values[4],period_length+1).to(device)
    timegrid_6 = torch.linspace(maturity_values[4],maturity_values[5],period_length+1).to(device)
    timegrid = torch.cat([timegrid_1,timegrid_2[1:period_length+1],timegrid_3[1:period_length+1],timegrid_4[1:period_length+1],timegrid_5[1:period_length+1],timegrid_6[1:period_length+1]],0)
    
    timegrid_daily = torch.linspace(0,maturity_values[n_maturities-1],n_steps_daily+1).to(device)
    print('timegrid daily', timegrid_daily)
    rate_1 = rates[0]*torch.ones_like(timegrid_1[0:period_length])
    rate_2 = rates[1]*torch.ones_like(rate_1)
    rate_3 = rates[2]*torch.ones_like(rate_2)
    rate_4 = rates[3]*torch.ones_like(rate_3)
    rate_5 = rates[4]*torch.ones_like(rate_4)
    rate_6 = rates[5]*torch.ones_like(timegrid_1)
    rate_1_daily = rates[0]*torch.ones_like(timegrid_daily[0: maturities_daily[0]])
    rate_2_daily = rates[1]*torch.ones_like(timegrid_daily[maturities_daily[0]: maturities_daily[1]])
    rate_3_daily = rates[2]*torch.ones_like(timegrid_daily[maturities_daily[1]: maturities_daily[2]])
    rate_4_daily = rates[3]*torch.ones_like(timegrid_daily[maturities_daily[2]: maturities_daily[3]])
    rate_5_daily = rates[4]*torch.ones_like(timegrid_daily[maturities_daily[3]: maturities_daily[4]])
    rate_6_daily = rates[5]*torch.ones_like(timegrid_daily[maturities_daily[4]-1: maturities_daily[5]])
    rate = torch.cat([rate_1,rate_2,rate_3,rate_4,rate_5,rate_6],0) # piecewise constant interest rate for each maturity
    rate_daily = torch.cat([rate_1_daily,rate_2_daily,rate_3_daily,rate_4_daily,rate_5_daily,rate_6_daily],0)
    seed=901
    
    torch.manual_seed(seed) # fixed for reproducibility
    model = Net_LSV(strikes=strikes, device=device, n_maturities=n_maturities)
    model.to(device)
    ...
    z_test_daily = np.random.normal(size=(MC_samples_price, n_steps_daily))
    z_test_var_daily = np.random.normal(size=(MC_samples_var, n_steps_daily))
    ...
    z_test_daily = torch.tensor(z_test_daily).to(device=device).float()
    z_test_var_daily = torch.tensor(z_test_var_daily).to(device=device).float()
    ...
    CONFIG = {"batch_size":50000,
              ...
              "n_steps":n_steps,
              "n_steps_daily":n_steps_daily,
              "target_data":data,
              "mu":mu,
              "seed":seed}
    
    model = train_nsde(model, z_val,z_val_var,z_test,z_test_var,z_test_daily,z_test_var_daily, CONFIG)
