...
class Net_LSV(nn.Module):
    ...
    def forward(self, S0, rate_daily, z, MC_samples, ind_T,n_maturities, maturities, timegrid): 
        """
...
def train_nsde(model,z_val,z_val_var,z_test,z_test_var,config):
    ...
    for epoch in range(n_epochs):
        ...
        if epoch==0:
            ...
            with torch.no_grad():
                pv_h, _,pe_u,pe_h,_, _,martingale_test, put_atm, call_atm, put_call_parity_error = model(S0, rate_daily, z_val, MC_samples_price, batch_steps_daily,n_maturities=n_maturities,maturities=maturities_daily,timegrid=timegrid_daily)
                _, var_pv_h,_,_,pe_var_u, pe_var_h,_,_,_,_ = model(S0, rate_daily, z_val_var,MC_samples_var, batch_steps_daily,n_maturities=n_maturities,maturities=maturities_daily,timegrid=timegrid_daily)                 
        ...
        for i in range(0,20*batch_size, batch_size):
            ...
            if epoch%10==1: 
                ...
                _, var_pv_h, _,_,_, _, _, _, _, _ = model(S0, rate_daily, batch_x1, batch_size_hedge,T_daily,n_maturities=n_maturities,maturities=maturities_daily,timegrid=timegrid_daily)
            ...
            if epoch%10!=1 and epoch%10!=2: 
                ...
                pv_h, _, _,pe_h,_, _, _, put_atm, call_atm, put_call_parity_error = model(S0, rate_daily, batch_x1, batch_size,T_daily,n_maturities=n_maturities, maturities=maturities_daily,timegrid=timegrid_daily)
                time_forward = time.time() - init_time
                pred = torch.reshape(pv_h,(n_maturities,n_strikes))
            ...
            if epoch%10==2: 
                ...
                init_time = time.time()
                batch_x1 = torch.randn(batch_size_hedge, batch_steps_daily, device=device)
                _, _, _,_, _,pe_var_h,_, _, _, _ = model(S0, rate_daily, batch_x1, batch_size_hedge,T_daily,n_maturities=n_maturities, maturities=maturities_daily,timegrid=timegrid_daily)
                time_forward = time.time() - init_time
                loss= pe_var_h
                init_time = t
                itercount +=1
        ...
        with torch.no_grad():
            pv_h_val, _,pe_u_val, pe_h_val,_, _,martingale_test, put_atm, call_atm, put_call_parity_error = model(S0, rate_daily, z_val, MC_samples_price, T_daily, n_maturities=n_maturities, maturities=maturities_daily,timegrid=timegrid_daily)
            _, var_pv_h_val,_,_, pe_var_u_val,pe_var_h_val, _,_,_,_ = model(S0, rate_daily, z_val_var,  MC_samples_var, T_daily, n_maturities=n_maturities, maturities=maturities_daily,timegrid=timegrid_daily)
        ...
        if  epoch==(n_epochs-1):
            ...
            with torch.no_grad():
                pv_h_test,_,pe_u_test, pe_h_test,_, _,martingale_test, put_atm, call_atm, put_call_parity_error = model(S0, rate_daily, z_test, MC_samples_price, T_daily, n_maturities=n_maturities,maturities=maturities_daily,timegrid=timegrid_daily)
                _, var_pv_h_test,_,_, pe_var_u_test,pe_var_h_test, _,_,_,_ = model(S0, rate_daily, z_test_var,  MC_samples_var, T_daily, n_maturities=n_maturities,maturities=maturities_daily,timegrid=timegrid_daily)
