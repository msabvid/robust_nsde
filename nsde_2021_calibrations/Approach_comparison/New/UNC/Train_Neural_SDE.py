...
def train_models(args):
    ...
    for idx, T in enumerate(maturities):
       ...
       base_learing_rate = 0.0001 # set maximum learning_rate so that model trains (if too high learning rate is chosen model parameters get stuck in some local minimum)
       ...
       for epoch in range(n_epochs):
            ...
            for i in range(0,20*batch_size, batch_size):
                ...
                if epoch<250 :
                   ...
                   init_time=time.time()
                   pv_1cv, var_pv_1cv,_, exotic_prc_no_cv,_,_, _, _, _, _, _ = model(S0, rate, batch_x1, batch_size,T, period_length=16,cal_exotic_var=0)
