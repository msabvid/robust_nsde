...
class Net_LV(nn.Module):
    ...
    def __init__(self, timegrid, strikes_call, device, n_maturities):
        ...
        self.net_Diffusion =  Net_timegrid(dim=3, nOut=1, n_layers=3, vNetWidth=100, n_maturities=n_maturities, activation_output="softplus")
        self.net_CV = Net_timegrid(dim=2, nOut=len(strikes_call), n_layers=3, vNetWidth=100, n_maturities=n_maturities, activation_output="softplus")
    ...
    def forward(self, S0, rate, z_1, MC_samples, ind_T, period_length,cal_exotic_var): 
        ...
        for i in range(1, ind_T+1):
            ...
            if cal_exotic_var==0:
                diffusion = self.net_Diffusion.forward_idx(idx_net, torch.cat([t,S1exp_old,V_old],1))
                volatility = self.V_vol.forward_idx(idx_net,V_old)
                drift = self.V_drift.forward_idx(idx_net,V_old)
                vol = self.V_vol.forward_idx(idx_net,V_old)
                cv_fwd = self.net_CV.forward_idx(idx_net, torch.cat([t,S1exp_old.detach()],1))
            if cal_exotic_var==1:
               with torch.no_grad():
                diffusion = self.net_Diffusion.forward_idx(idx_net, torch.cat([t,S1exp_old,V_old],1))
                volatility = self.V_vol.forward_idx(idx_net,V_old)
            ...
            V_new = V_old + drift.reshape(MC_samples,1)*h + volatility.reshape(MC_samples,1)*dB
            ...
            cv_van_1 += cv_fwd * repeat_1 #cv_fwd[:,1:1+len(self.strikes_call)] * repeat_1
            if cal_exotic_var==1:
...
def train_models(args):
    ...
    for idx, T in enumerate(maturities):
       ...
       params_SDE = list(model.net_Diffusion.parameters()) + [model.rho, model.v0] + list(model.V_drift.parameters())+list(model.V_vol.parameters())
       params_CV = list(model.net_CV.parameters())
       params_hedge = list(model.exotic_hedge.parameters())

       optimizer_SDE = torch.optim.Adam(params_CV,lr=base_learing_rate)
       ...
       for epoch in range(n_epochs):
            ...
            for i in range(0,20*batch_size, batch_size):
                ...
                if epoch%2==0 and epoch<250: 
                    ...
                    model.V_vol.freeze()
                    init_time = time.time()
                    _, var_pv_1cv,_, _,_,_, _, _, _, _, _ = model(S0, rate, batch_x1, batch_size,T, period_length=16,cal_exotic_var=0)
                ...
                if epoch%2==1 and epoch<250:
                    ...
                    model.net_CV.freeze()
                    init_time = time.time()
                    pv_1cv, _,_, exotic_prc_no_cv,_,_, _, _, put_atm, call_atm, put_call_parity_error = model(S0, rate, batch_x1, batch_size,T, period_length=16,cal_exotic_var=0)
                    time_forward = time.time() - init_time
                    pred = torch.reshape(pv_1cv,(6,21))
                    MSE = loss_fn(torch.mul(pred,vega),torch.mul(target_mat_T,vega))
                    loss= 0.5*exotic_prc_no_cv + LAMBDA * MSE + c_2/2 * MSE**2  # we use exotic price without control variate in optimising payoff bound 
                    init_time = time.time()
                    itercount_2 +=1
                    loss.backward()
                    if itercount_2%20==0:
                ...
                if epoch>=250:
                    ...
                    init_time = time.time()
                    _, _,_, _,_, _,exotic_prc_var_cv,_, _, _, _ = model(S0, rate, batch_x1, batch_size_hedge,T, period_length=16,cal_exotic_var=1)
                    time_forward = time.time() - init_time
            ...
            if (epoch>= 250 and exotic_hedge_error_change < 0.001) or epoch==274:
                  ...
                  with torch.no_grad():
                    pv_1cv, _,_,exotic_prc_no_cv, exotic_prc_cv,_, _,martingale_test, put_atm, call_atm, put_call_parity_error = model(S0, rate, z_test, MC_samples_gen, T, period_length=16,cal_exotic_var=1)
                    _, var_pv_1cv_val,_,_,_, exotic_prc_var_no_cv_val,exotic_prc_var_cv_val, _,_,_,_ = model(S0, rate, z_test_var,  MC_samples_var, T, period_length=16,cal_exotic_var=1)
                  print("pred_1:",pv_1cv)
                  print("target:", target_mat_T )
                  pred=torch.reshape(pv_1cv,(6,21))
                  print('pred', pred)
