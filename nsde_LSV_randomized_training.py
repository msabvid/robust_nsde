...
def train_nsde(model, z_test, config):
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(0,20*batch_size, batch_size):
            nets_frozen = np.random.choice(len(maturities), size=len(maturities)-1, replace=False)
            if requires_grad_SDE:
                for j in nets_frozen:
                    model.diffusion.freeze(j)
                    model.driftV.freeze(j)
                    model.diffusionV.freeze(j)
            ...
            if requires_grad_CV:
                ...
                loss = len(maturities) * MSE # unbiased estimator for randomized training
                init_time = time.time()
                loss.backward()
