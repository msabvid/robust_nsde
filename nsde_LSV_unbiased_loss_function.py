...
class Net_LSV(nn.Module):
    ...
    def forward(self, S0, z, MC_samples, ind_T, period_length=30): 
        ...
        return price_vanilla_cv, var_price_vanilla_cv, exotic_option_price, exotic_option_price.mean(), exotic_option_price.var(), 0 
...
def train_nsde(model, z_test, config):
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(0,20*batch_size, batch_size):
            ...
            init_time = time.time()
            pred1, _, _, exotic_option_price, _, _ = model(S0, batch_z, batch_size,T, period_length=16)
            pred2, _, _, _, _, _ = model(S0, torch.randn_like(batch_z), batch_size,T, period_length=16)
            ...
            print('iteration {}, loss={:4.2e}, exotic price={:.4f}, time_forward={:.4f}, time_backward={:.4f}'.format(itercount, loss.item(), exotic_option_price.item(), time_forward, time_backward))
            optimizer_SDE.step()
        ...
        with torch.no_grad():
            pred, _, _, exotic_option_price_mean, _, _ = model(S0, z_test, z_test.shape[0], T, period_length=16)
            print("pred:",pred)
        ...
        if loss_val < loss_val_best:
             ...
             checkpoint = {"state_dict":model.state_dict(),
                     "exotic_price_mean": exotic_option_price_mean,
                     "T":T,
                     "pred":pred,
